...
\usepackage{titlesec}
\usepackage{amsmath} % Needed for commands like \tfrac
\usepackage{xcolor}
\usepackage{listings}
\lstset{
    ...
    inputencoding=utf8,
    extendedchars=true,
    literate={✓}{{$\checkmark$}}1 {🚀}{{$\rightarrow$}}1 {❌}{{X}}1 {📋}{{L}}1 {🔄}{{R}}1 {🎉}{{!}}1 {📁}{{F}}1
}
\usepackage{float}
...
\usepackage[final]{microtype} % Better line breaking, reduces overfull hboxes (load after footnotes)
...
% Removed undefined macro \chaptertitlefont (not provided by packages in use)
...
\input{Front.tex} % FRONT PAGE
\input{Dedication.tex} % Dedication page
\input{Acknowledgment.tex} % Acknowledgments page
...
\input{Chapters/robotsandagriculture}
\input{Chapters/Requirements}
\input{Chapters/kinematics and Dynamics}
\input{Chapters/manupulator}
\input{Chapters/gripperDesign}
